...
def save_data_to_sheet(sheet_id, df):
    try:
        ...
        st.cache_data.clear()  # Czyszczenie cache po zapisie
    except Exception as e:
...
with tab_logi:
    ...
    if typ_logu == "Wszystkie Wyjazdy / Logi WZ":
        ...
        st.dataframe(get_data_from_sheet(ID_KOREKTY), use_container_width=True)import os
import io
import smtplib
from email.message import EmailMessage
from datetime import datetime
...
# Autoryzacja w Google Cloud za pomocą Secrets
@st.cache_resource
def get_gspread_client():
    scopes = ["https://www.googleapis.com/auth/spreadsheets"]
    creds_dict = dict(st.secrets["gcp_service_account"])
...
def get_data_from_sheet(sheet_id):
    try:
        ...
        st.error(f"Błąd odczytu z Google Sheets ({sheet_id}): {e}")
        return pd.DataFrame()
